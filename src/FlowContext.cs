...
public sealed class FlowContext : ApplicationContext
{
    ...
    private async Task InitTranscriberAsync()
    {
        try
        {
            ...
            if (!File.Exists(model))
            {
                ...
                _tray.ShowBalloonTip(4000, "Flow", "First run: downloading the speech model (~148 MB). One-time.", ToolTipIcon.Info);
                bool ok = await TryDownloadModelAsync(model);
                if (!ok || !File.Exists(model))
                {
                    SetStatus("Model not found");
                    _tray.ShowBalloonTip(9000, "Flow",
            ...
            _tray.ShowBalloonTip(3000, "Flow is ready",
                $"Hold {_settings.Modifier} + {_settings.Key} and speak.", ToolTipIcon.Info);
        }
        catch (Exception ex)
        {
            SetStatus("Model failed to load");
            _tray.ShowBalloonTip(6000, "Flow error", ex.Message, ToolTipIcon.Error);
        }
    }
    ...
    private void StartCapture(CaptureMode mode)
    {
        ...
        if (!_modelReady)
        {
            _tray.ShowBalloonTip(2000, "Flow", "Still loading the model, one sec…", ToolTipIcon.Info);
            return;
        }
    ...
    private async void StopCapture()
    {
        ...
        {
            _tray.ShowBalloonTip(4000, "Flow error", ex.Message, ToolTipIcon.Error);
        }
        finally
        {
            _overlay.HideState();
    ...
    private void BuildMenu()
    {
        ...
        _statusItem = new ToolStripMenuItem("Flow — loading…") { Enabled = false };
        ...
        var openItem = new ToolStripMenuItem("Open Flow", null, (_, _) => OpenMain());
        ...
        menu.Items.Add(new ToolStripSeparator());
        menu.Items.Add(new ToolStripMenuItem("Quit Flow", null, (_, _) => Quit()));
    ...
    private void ReloadData(bool notify)
    {
        ...
        if (notify) _tray.ShowBalloonTip(2000, "Flow", "Dictionary and snippets reloaded.", ToolTipIcon.Info);
    }
    ...
    {
        _statusItem.Text = $"Flow — {status}";
        _tray.Text = $"Flow — {status}";
    }
